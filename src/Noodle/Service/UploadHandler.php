...
class UploadHandler implements ServiceLocatorAwareInterface
{
    ...
    function __construct($fileBankService = null, $options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/_data/',
            'upload_url' => $this->get_full_url().'/_data/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
        ...
        if($fileBankService){
        	$this->setFilebankService($fileBankService);
        }
        if ($options) {
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        // FileBank implementation
        $fileEntity = $this->getFilebankService()->save($file_path);
		$file->name = $fileEntity->getId();
    ...
    public function setFilebankService($filebankService)
    {
    	$this->filebankService = $filebankService;
    }
